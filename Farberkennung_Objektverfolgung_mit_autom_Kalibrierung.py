...
#import datenerhebung2_praesentation as d2p
#import datenerhebung3_praesentation as d3p
#mport momentan as akprog
...
def dynamik(pos_alt, pos_neu): # Diese Funktion versucht die Groesse der Bewegung zu ermitteln
	c = np.sqrt(((pos_alt[0]-pos_neu[0])**2)+((pos_alt[1]-pos_neu[1])**2))
	#print c
	if c>200:
		return 3
...
def turtle_weiterzeichnen(versch_vec):
	#turtle.fd(10)
	x_versch,y_versch = versch_vec[-1]
	x_mom, y_mom = turtle.position()
	turtle.goto(x_mom+x_versch*50,y_mom+y_versch*50)
...
def main():
	...
	while(True):
		...
		zeichne_kreis(frame,color_rec(frame)) #Kreis wird an aktueller Taktstockposition gezeichnet
	...
	#plotten2d_ableitung(daten, 3, versch_vec)
	cap.release()
